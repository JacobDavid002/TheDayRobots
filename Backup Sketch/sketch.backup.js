...
function draw() {
  ...
  if (startTutorial == false && startGame == false) {
    ...
    if (Startgametimer.time < 11){
      ...
      imageMode(CENTER);
      image(IntroFilm , width/2, height/2);
...
function EnemySpawn() {
  ...
  if (gameTimer.get() > spawnTimer) {
    ...
    RanImage = random(robohands);
    enemys[currentEnemy] = new Enemy(RanX, -50, 50, 50, 0.0, 0, RanX, RanX, RanImage);
    currentEnemy++;
...
function EndGame(){
  ...
  rectMode(CENTER);
  fill('#000000');
  rect(width / 2, height / 2, width - 900, height - 700);
  pop();
